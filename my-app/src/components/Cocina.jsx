import React, { useEffect, useState, Fragment } from "react";
import { BrowserRouter as Router, Switch, Route, Link } from "react-router-dom";

import Meseros from "./Meseros";
import { db } from "./firebase";
import Fecha from "./Fecha";


const Cocina = () => {
  // TODO: Cambiar de nombre a  "orders", "setOrders"
  const [orders, setOrders] = useState([]);

  const getOrders = async () => {
    await db
      .collection("orders")
      .get()
      .then((querySnapshot) => {
        const docs = [];
        querySnapshot.forEach((comanda) => {
          docs.push({ ...comanda.data(), id: comanda.id });
        });
        console.log(docs);
        setOrders(docs);
      });
  };

  const updateOrder = (orderId, status) => {
    db.doc(`/orders/${orderId}`)
      .update({ status, preparedAt: new Date() })
      .then(() => {
        setOrders(
          orders.map((order) =>
            order.id === orderId
              ? { ...order, status, preparedAt: new Date() }
              : order
          )
        );
      });
  };

  useEffect(() => {
    getOrders();
    //console.log(getOrders);
  }, []);

  console.log(orders);

  const components = orders.map((order) => {
    return (
      <span>
        {/* {order.status ? (
          <Fragment key={order.id}>
            {" "}
            <table className="showComponents-verde">
              <tr>
                <th>Cliente:{order.cliente}</th>
                <th>Numero de mesa:{order.nroMesa}</th>
              </tr>

              <tr>
                {order.platillos.map((platillo, idx) => (
                  <tr key={`${order.id}-${platillo.nameProduct}-${idx}`}>
                    {platillo.nameProduct}
                  </tr>
                ))}
              </tr>
              <button className="button">Estado</button>
              <br></br>
              <br></br>
            </table>
          </Fragment>
        ) : (
          <Fragment key={order.id}>
            {" "}
            <table className="showComponents">
              <tr>
                <th>Cliente:{order.cliente}</th>
                <th>Numero de mesa:{order.nroMesa}</th>
                <th>
                  {" "}
                  <Fecha />{" "}
                </th>
              </tr>

              <tr>
                {order.platillos.map((platillo, idx) => (
                  <tr key={`${order.id}-${platillo.nameProduct}-${idx}`}>
                    {platillo.nameProduct}
                  </tr>
                ))}
              </tr>
              <button className="button" onClick={() => updateOrder(order.id)}>
                Cambiar a preparada
              </button>
              <br></br>
              <br></br>
            </table>
          </Fragment>
        )} */}
      </span>
    );
  });

  //   const OrdersComponentes = ({ count }) => (
  //     <div>
  //       {count.map((a) => (
  //         <table>
  //           <tr>
  //             <td>{a.cliente}</td>
  //             <td>{a.numesa}</td>
  //             <td>{a.order}</td>
  //           </tr>
  //         </table>
  //       ))}
  //     </div>
  //   );

  //   const components = count.map ((orders,index) => (
  //     <OrdersComponentes count={count} key={`orders-${index}`} cliente={orders.cliente} numesa={orders.nroMesa} order={orders.platillos} />

  //     )
  //   )

  return (
    <Fragment>
      <div className="log">
        <div className="logo"></div>
        <div className="logoB"></div>
        <div className="logoY"></div>
        <img
          className="logoImg"
          src="data:image/png;base64,"
        />
      </div>

      <div className="contentKitchen">
        {/* {components} */}
        {orders.map(order => (
          <article key={order.id} className="eachOrder">
             <Fecha />
            <header>
              <h1>Cliente: {order.cliente}</h1>
              <h1>Numero de mesa: {order.nroMesa}</h1>
            </header>
           
            <div>
              {order.platillos.map((platillo, idx) => (
                <div key={`${order.id}-${platillo.nameProduct}-${idx}`}>
                  {platillo.nameProduct}
                </div>
              ))}
            </div>
            <br/>
            {order.status === 'placed'
              ? <button onClick={() => updateOrder(order.id, 'prepared')} className="button is-danger">Cambiar a preparada</button>
              : order.status === 'prepared'
                ? <button onClick={() => updateOrder(order.id, 'delivered')} className="button is-warning">Cambiar a entregada</button>
                : <span className="button is-success">Entregada</span>
            }
          </article>
        ))}
      </div>
    </Fragment>
  );
};
















export default Cocina;
